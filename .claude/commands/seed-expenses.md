...
Before generating anything, confirm the user_id exists in the users table. If not, stop and say: "No user found with id: <user-id>."
...
2. Uses these categories with realistic Indian descriptions and amounts (Rs):
    - Food: 50-800
    - Transport: 20-500
    - Bills: 200-3000
    - Health: 100-1500
    ...
    - Shopping: 200-5000
    - Other: 50-1000
3. Distributes categories roughly proportionaly (Food most common, Health and Entertainment least)
4. Uses the db connetion pattern from db.py - do not hardcode the database file name
5. Uses parameterised queries only - no string formatting in SQL
6. Insert all expenses in a single transaction - roll back everything if any isert fails
